...
namespace ASCOM.BrebissonV1.Focuser
{
    ...
    internal static class FocuserHardware
    {
        ...
        static FocuserHardware()
        {
            ...
            try
            {
                using (Profile driverProfile = new Profile())
                {
                    ...
                    double v; string s = driverProfile.GetValue(DriverProgId, "StepSize", string.Empty, "10");
                    if (double.TryParse(s, NumberStyles.Float, CultureInfo.InvariantCulture, out v)) StepSize= v;
                    int i;
                    if (int.TryParse(driverProfile.GetValue(DriverProgId, "slowSpeed", string.Empty, "10"), out i)) slowSpeed = i;
                    if (int.TryParse(driverProfile.GetValue(DriverProgId, "fastSpeed", string.Empty, "2000"), out i)) fastSpeed = i;

                    s= driverProfile.GetValue(DriverProgId, "savedPos", string.Empty, "");
                    string []p = s.Split(',');
                    for (int j=0; j<p.Length; j++)
                    {
                        ...
                        if (k != -1)
                        {
                            CSavedPos sp = new CSavedPos();
                            sp.name = p[j].Substring(0, k);
                            if (int.TryParse(p[j].Substring(k+1), out sp.pos)) savedPos.Add(sp);
                        }
                    }
        ...
        public class CSavedPos
        {
            public string name;
            public int pos;
        };
        public static List<CSavedPos> savedPos= new List<CSavedPos>();
        ...
        public static void saveProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                driverProfile.WriteValue(DriverProgId, "StepSize", StepSize.ToString(CultureInfo.InvariantCulture));
                ...
                string poses= "";
                foreach (CSavedPos pos in savedPos) poses += pos.name + "_" + pos.pos.ToString() + ",";
                driverProfile.WriteValue(DriverProgId, "savedPos", poses);
            }
        }
        ...
        internal static void Halt() { SharedResources.SendSerialCommand(":FQ#", 0); SharedResources.doLog("Foc stop"); } /// Immediately stop any focuser motion due to a previous <see cref="Move" /> method call.
        ...
        {
            SharedResources.SendSerialCommand(":SN"+Position.ToString("X4")+"#", 0);
            SharedResources.SendSerialCommand(":FG#", 0);
            SharedResources.doLog("Foc move to "+Position.ToString()); 
        }
        public static void moveIn(int spd) { SharedResources.SendSerialCommand(":Fi" + spd.ToString() + '#'); }
        public static void moveOut(int spd) { SharedResources.SendSerialCommand(":Fo" + spd.ToString() + '#'); }
        internal static void Stop() { SharedResources.SendSerialCommand(":FQ#", 0); SharedResources.doLog("Foc stop"); }
        internal static int Position { get { return SharedResources.FocusserPosition; } } // Return the focuser position
        private static double _StepSize = 10;
